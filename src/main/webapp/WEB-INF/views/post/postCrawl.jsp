<%@ page contentType="text/html; charset=UTF-8"
		import="java.util.*, mvc.modelpost.*"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt"%>

<jsp:useBean id="CrawlDAO" class="mvc.modelpost.CrawlDAO" scope="session" />
<jsp:useBean id="CrawlDO" class="mvc.modelpost.CrawlDO" />

<%
	ArrayList<CrawlDO> list = CrawlDAO.getAllCrawl();
	String	result = "";	
%>
						...
						<section id="container">
							<form role="form" method="post" action="/post/write">
								<table>
									...
									<%for(CrawlDO tDO : list){%>
						                <tr>
											<td><a href="<%=tDO.getUrl()%>"><%=tDO.getTitle() %></a></td>
											<td><%=tDO.getNickName() %></td>
											<td><%=tDO.getViews() %></td>
											<td><%=tDO.getDate() %></td>
											<td><%=tDO.getSiteName() %></td>
										</tr>
									<%} %>
								</table>
							</form>
